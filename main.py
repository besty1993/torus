...
class Render():
    ...
    def getDuplicateIdxs(self, array):
        df = pd.DataFrame(array).copy()
        df[0] = 1000*df[0]+df[1]
        return df.groupby([0]).indices

    def filterByDistance(self, proj, bright):
        """
        ...
        dups_list = self.getDuplicateIdxs(proj)
        for dups in dups_list.values():
            if len(dups) == 1:
                idx = dups[0]
            else:
                z_dists = rot_points[dups,2]
...
if __name__ == '__main__':
    torus = Torus(R=CONFIG.R, r=CONFIG.r, a=1, b=0.5, c=0.7)
    ...
    for t in range(100):
        ...
        bright = renderer.computeBrightness(rot_norms)

        filtered_proj, filtered_bright = renderer.filterByDistance(proj, bright)
        
        print(renderer.print(filtered_proj, filtered_bright))
        time.sleep(0.1)